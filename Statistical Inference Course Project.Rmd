...
Specification of the two datasets used in this demonstration, as follow:  
...
h <- h + geom_histogram(color = "black", fill = "white", bins = n) +
        ...
        geom_vline(aes(xintercept = mean(x = X_mu), color = "X_mu"), lty = 1, lwd = 1) +
        geom_vline(aes(xintercept = mu, color = "mu"), lty = 2, lwd = 0.8) +
...
In Figure-2, the histogram graph exhibits a symmetrical (almost) bell-shape density curve overlapping on the histogram suggesting the $X_\mu$ dataset approximates normal distribution. Mean $\bar{X_\mu}$ = `r round(mean(e$X_mu), 4)` (green vertical line) is said to be statistically equivalent to theoretical mean $\mu$ (blue vertical line), implying that the sample mean is the true mean of the population and this assumption is reinforced by the sample variance $\sigma_{X_\mu}^2 =$ `r round(var(e$X_mu), 4)` and sample standard error $SE_{X_\mu} =$ `r round(sd(e$X_mu)/sqrt(B), 4)` respectively signify the data points of $X_\mu$ will converge around the theoretical mean. 
...
Note that the green vertical line plotted on the histogram graph in Figure-2 represents the sample mean $\bar{X_\mu}$ which is positioned ever so closely next to the theoretical mean $\mu$ (blue vertical line), where $t.value$ will tells us if $\bar{X_\mu}$ is less than or greater than $\mu$.  
&nbsp;
...
h <- h + 
    ...
    geom_density(alpha = 0.5, fill = "#FF6666", col = "red") +
    geom_vline(aes(xintercept = mean(x = X_e), color = "X_e"), lty = 1, lwd = 1) +
    geom_vline(aes(xintercept = mu, color = "mu"), lty = 2, lwd = 0.8) +
...
Boxplot graph in Figure-3 and histogram (with density curve) graph in Figure-4 are plotted based on the same $X_e$ dataset, clearly they don't look Gaussian at all but are skewed (right skewed in this case). This phenomenon explains that only the sampling statistic e.g. sampling means will approximate normal distribution for adequately large sample size irregardless of the underlying data distribution. Having said that, the as-is data in the sample dataset will not approximate Gaussian distribution irregardless of size.  

Note that mean $\bar{X_e}$ and variane $\sigma_{X_e}^2$ of the dataset $X_e$ are both echoing the theoretical mean $\mu$ (blue vertical line) and $\sigma$, whereas only the $\bar{X_\mu}$ (green vertical line) mimic the theoretical mean but $\sigma_{X_\mu}^2$ does not resonate the theoretical variance, which is in line with our prior supposition about the population distribution is not Gaussian compare to the sample distribution.  
&nbsp;
