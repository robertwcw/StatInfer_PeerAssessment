...
|------------------------|---------------------------------------------------|
| $\mu_0$ = `r round(mean(tg$len), 4)` | Theoretical mean of tooth length |  
| $\sigma_0^2$ = `r round(var(tg$len), 4)` | Theoretical variance of tooth length |  
...
Though the efficacy of the supplements has seen positive result in promoting tooth growth for both **OJ** type and **VC** type as exhibited in the scatter plots in Figure-8, red curve on the left and blue curve on the right depicting the average growth rate for **OJ** supplement and **VC** supplement respectively. Note that average growth rate for **OJ** is more rapid at lower than mean $\mu_d$ (yellow vertical line) dosage and tapered at above mean $\mu_d$ dosage, while average growth rate for **VC** is more uniform across the recommended dosage range. Referring to the table attached below, overall effectiveness of **OJ** is better in encouraging tooth growth at lower dosage (0.5 ~ 1 dose) compare to **VC** which is more potent at higher dosage (2 doses).  
&nbsp;

| Supplement x Dosage | Length (Mean, Min, Max) |  
|------------------------|---------------------------------------------------|  
...
Generally, we know that both supplements are effective for tooth growth, albeit varied rate at differing dosage. The average length of growth for each supplement type vs theoretical mean listed herein, **OJ** type spurs longer average tooth length than the hypothetical mean $\mu_0$, than **VC** type which tooth length is shorter than the hypothetical mean $\mu_0$.  
&nbsp;

| Supplement | Average Length vs hypothetical mean $\mu_0$ = `r round(mean(tg$len),4)` |  
|------------|-------------------------------|  
...
Let's take a look at the bootstrapped ToothGrowth sampling data distribution (Figure-9 & Figure-10), as oppose to its primal distribution (Figure-5 & Figure-6). It looks more Gaussian now.  
&nbsp;
