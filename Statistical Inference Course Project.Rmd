...
rm(ToothGrowth)
```
It has 60 rows with 3 variables stored in data frame.  
- len ......: Length of tooth  
- supp ...: supplement type {OJ $= 1$, VC $= 2$}  
- dose ...: dosage {$\frac{1}{2}$, $1$ or $2$}
&nbsp;
```{r eda.ToothGrowth.1}
# reveal the data structure
str(tg)
```
&nbsp;

#### Exploratory data analysis on ToothGrowth
&nbsp;
...
|------------------------|---------------------------------------------------|
| $\mu_l$ = `r round(mean(tg$len), 4)` | Theoretical mean of tooth length |  
| $\sigma_l^2$ = `r round(var(tg$len), 4)` | Theoretical variance of tooth length |  
| $\mu_d$ = `r round(mean(tg$dose), 4)` | Theoretical mean of supplement dosage |  
| $\sigma_d^2$ = `r round(var(tg$dose), 4)` | Theoretical variance of supplement dosage |  
&nbsp;
...
The exploratory graph plots (Figure-5 & Figure-6) show that the data distribution for ToothGrowth dataset does not look Gaussian.  
...
```
&nbsp;
There is an overall up trend in the average rate of tooth growth length on the test subjects administered with either **OJ** supplement or **VC** supplement. The test subjects were separated into two groups of 30 participants each by supplement type. In Figure-7, the green dotted smooth trend line on the scatter plot exhibits a phenomenon where the test subjects given lower dosage of between $\frac{1}{2}$ to $1$ dose experienced higher average tooth growth rate than test subjects given higher dosage of greater than $1$ dose experienced lower average growth rate.  
...
oj05 <- subset(tg, (dose == 1/2 & as.numeric(supp) == 1),len)$len
oj10 <- subset(tg, (dose == 1 & as.numeric(supp) == 1),len)$len
oj20 <- subset(tg, (dose == 2 & as.numeric(supp) == 1),len)$len
vc05 <- subset(tg, (dose == 1/2 & as.numeric(supp) == 2),len)$len
vc10 <- subset(tg, (dose == 1 & as.numeric(supp) == 2),len)$len
vc20 <- subset(tg, (dose == 2 & as.numeric(supp) == 2),len)$len
```
&nbsp;

The efficacy of the supplements has seen positive result in promoting tooth growth for both **OJ** type and **VC** type as exhibited in the scatter plots in Figure-8, red smooth curve on left and blue smooth curve on the right depicting the average growth rate for **OJ** supplement and **VC** supplement respectively. **OJ** supplement is more effective for tooth growth at lower dosage

| Supplement x Dosage | Mean $\&$ Range |  
|------------------------|---------------------------------------------------|  
...
```{r}
# tg.oj <- subset(tg, as.numeric(supp) == 1)  #"OJ" supplement
# tg.vc <- subset(tg, as.numeric(supp) == 2)  #"VC" supplement
# B.tg.oj <- bootstrap(tg.oj, B/2)  
# B.tg.vc <- bootstrap(tg.vc, B/2)  
