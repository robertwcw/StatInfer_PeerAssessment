...
5-number summary of ToothGrowth.
```{r eda.ToothGrowth.2}
summary(tg)
```

```{r eda.ToothGrowth.3, warning=FALSE, fig.cap="Figure-5", fig.align="left"}
...
```
&nbsp;
There is an overall up trend observed in the rate of tooth growth for OJ supplement and VC supplement given to the test subjects separated into two groups of 30 subjects each. Nonetheless rate of growth varied on different dosage administered. In Figure-5, the red dotted smooth trend line on the scatter plot exhibits higher growth rate for lower dosage than slower rate when higher dosage was administered on the participants.  

```{r eda.ToothGrowth.4, warning=FALSE, fig.cap="Figure-6", fig.align="left"}
par(mfrow = c(1,2))
